...
def parse_json_blocks(text: str) -> List[dict]:
    ...
    if not blocks:
        try:
            ...
            text = text.strip()
            logger.info(f"Bare JSON attempt, first 50 chars: {repr(text[:50])}")  # add this
...
async def run_simulation():
    ...
    followed by routes read directly from session state once the route agent finishes.
    """
    ...
    # Accumulate incidents across all data agents
    all_incidents = []
    ...
    async def run_pipeline():
        try:
                    ...
                    if event.content and event.content.parts:
                        for part in event.content.parts:
                            if hasattr(part, "text") and part.text:
                                ...
                                # Only parse JSON for the three data agents
                                # Routes come from session state, not text output
                                            ...
                                            if isinstance(block, list):
                                                for idx, item in enumerate(block):
                                                    if "id" in item or "text" in item:
                                                        ...
                                                        # so the frontend renders them one by one
                                                        await queue.put({
                                                            "type": "incident",
                                                            "agent": agent_name,
                                                            "incident": {
            ...
            # Read routes directly from session state — the route agent stores them there
            # via compute_optimal_routes() so we never have to parse large JSON from text output
            current_session = await session_service.get_session(
                app_name="dispatch_app",
                user_id=user_id,
                session_id=session_id
            )
            routes = current_session.state.get("computed_routes", [])
            logger.info(f"Routes read from session state: {len(routes)}")
            ...
            if routes:
                ...
                logger.warning("No routes found in session state")
